...
#### 4889
- {}와 같이 두개의 문자로만 이루어진 쌍이 있다고 하자. 랜덤하게 문자열이 주어졌고, 문자들을 바꿔가며 특정 조건을 만족하는 문자열로 만들려고 할 때, 문자를 바꿔야 하는 최소 횟수를 구하여야 하는 문제였다. stack을 활용하는 쉬운 문제였지만 딱 하나 추가적으로 고려해야할 사항이 있었다. 이는 stack에 }을 쌓아야할 때 {로 애초에 바꿔서 넣어버리는 것이었다. 애초에 문자 자체를 미리 바꿔버리고 생각하는 것. 이게 말로는 설명이 잘 안되는데 풀이를 보면 이해가 되고, 괜찮은 접근법 중 하나라고 생각한다.
